...
This module implements three detection methods for identifying dangerous Windows
kernel APIs in disassembled IOCTL handlers:
  1. String Matching - Fast detection via API name in comments/strings
  2. Call Instruction Analysis - Pattern-based call instruction detection
  3. String Reference Detection - Identifies suspicious string constants

Author: Person 2 (API Hunter)
Last Updated: 2025-11-16
Integration: Called by plugins/driver_analysis.py
"""
            ...
            if any(reg in op_str.lower() for reg in ['rax', 'rbx', 'rcx', 'rdx', 'rsi', 'rdi', 'r8', 'r9', 'r10', 'r11']):
                ...
                if suspicious:
                    findings.append({
                        ...
                        'category': 'UNKNOWN',
                        'risk': 5,
                        'why_dangerous': 'Indirect call via register after loading user data - potential dynamic API resolution'
                    })
...
def find_dangerous_apis(disassembly_lines):
    """
    Main API scanner function - integrates all three detection methods.

    This is the primary function called by driver_analysis.py.
    ...
    Returns:
        list: Comprehensive list of all findings from all methods
              Each finding is a dict with keys:
                ...
                - method: Detection method ('string', 'call_pattern', 'string_reference')
                - confidence: 0.0-1.0 confidence score
                - address: Memory address where found
    ...
    # Deduplicate findings (same address + same API name)
    ...
    final_findings = sorted(deduplicated.values(), key=lambda x: x['address'])
